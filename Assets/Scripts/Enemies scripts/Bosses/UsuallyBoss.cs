...
public class UsuallyBoss : MonoBehaviour
{
    ...
    bool wave;
    public float timeAfterLastEnemy;// Время идти после последнего врага
    bool goLastenemy;
    ...
    private void FixedUpdate()
    {
        ...
        {
            float time = 0;
            if(enemies.Length != 0) switch (enemies[enemies.Length-1].tag)
            {
                case "UsuallyEnemy":
                    time = enemies[enemies.Length - 1].GetComponent<UsuallyEnemy>().timeAttack;
                    break;

                case "DoubleEnemy":
                    time = enemies[enemies.Length - 1].GetComponent<DoubleEnemy>().timeAttack;
                    break;

                case "CupEnemy":
                    time = enemies[enemies.Length - 1].GetComponent<CupEnemy>().timeAttack;
                    break;
            ...
            if(!wave) Invoke("Wave", timeWave);
            wave = true;

            Invoke("Go", timeAfterLastEnemy + time);
        }
